...
function mockSubmit(input: FulfillInput): SubmitResult {
  console.log(
    `[mock-fulfillment] order ${input.orderId}: ${input.items.length} item(s) → ${input.recipient.city}, ${input.recipient.country}`,
  );
...
async function printfulSubmit(
  ...
  if (!opts.confirm) {
    throw new Error(
      "PRINTFUL_CONFIRM_ORDERS must be true for automated fulfillment. Refusing to create draft Printful orders.",
    );
  }
  ...
  if (!res.ok) {
    throw new Error(`Printful POST /orders?confirm=true failed (${res.status}): ${json?.error?.message ?? "unknown error"}`);
  }
  const id = json?.result?.id;
  if (id == null) throw new Error("Printful POST /orders?confirm=true returned no order id");
      ...
      if (order.recipient_email) {
        try {
          await admin.emails.send({
            ...
            html: `<p>Thanks for your order!</p><p>Your custom item is being prepared for printing. We'll email you again when it ships.</p><p style="color:#888;font-size:12px">Order ${order.id}</p>`,
          });
        } catch (e) {
          console.error("order-received email failed", e);
        }
      }
