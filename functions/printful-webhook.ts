...
export default async function (req: Request): Promise<Response> {
  ...
  if (event.status === "fulfilled") {
    const row = (updated as Array<{ id: string; recipient_email: string | null }>)?.[0];
    if (row?.recipient_email) {
      try {
        await admin.emails.send({
          ...
          html: `<p>Good news — your custom item is on its way!</p><p style="color:#888;font-size:12px">Order ${row.id}</p>`,
        });
      } catch (e) {
